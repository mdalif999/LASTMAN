...
def stock_report_page(page: ft.Page):
    items = get_inventory_items(limit=99999)
    current_filter = [None]
    ...
    def get_stock_status(stock_val, unit_size, u_type):
        u_type = str(u_type).lower()
        if u_type == "alum" and unit_size > 0:
            pcs = stock_val / unit_size
            if pcs <= 0:
                return "Out of Stock", "red400"
            elif pcs < 10:
                return "Low Stock", "orange400"
            else:
                return "In Stock", "green400"
        else:
            if stock_val <= 0:
                return "Out of Stock", "red400"
            elif stock_val < 10:
    ...
    def get_stock_summary():
        ...
        for i in items:
            ...
            status, _ = get_stock_status(stock_qty, unit_size, u_type)
    ...
    stock_table = ft.DataTable(
        ...
        heading_row_color="#22ffffff",
    )

    def render_table(filter_status=None):
        current_filter[0] = filter_status
        ...
        for i in items:
            ...
            u_type = str(i[10]).lower()
            status, status_color = get_stock_status(stock_val, unit_size, u_type)
        ...
        content=ft.Column([
            ...
            ft.Row([
                ft.Container(
                    content=ft.Column([
                        ft.Text("Total Items"),
                        ft.Text(str(t_items), size=20, weight="bold")
                    ]),
                    bgcolor="#1e2b5e", padding=20, border_radius=10, expand=True,
                    on_click=lambda _: render_table(None),
                    ink=True,
                ),
                ft.Container(
                    content=ft.Column([
                        ft.Text("Low Stock", color="orange"),
                        ft.Text(str(l_stock), size=20, weight="bold")
                    ]),
                    bgcolor="#1e2b5e", padding=20, border_radius=10, expand=True,
                    on_click=lambda _: render_table("Low Stock"),
                    ink=True,
                ),
                ft.Container(
                    content=ft.Column([
                        ft.Text("Out of Stock", color="red"),
                        ft.Text(str(o_stock), size=20, weight="bold")
                    ]),
                    bgcolor="#1e2b5e", padding=20, border_radius=10, expand=True,
                    on_click=lambda _: render_table("Out of Stock"),
                    ink=True,
                ),
                ft.Container(
                    content=ft.Column([
                        ft.Text("Total Investment", color="green"),
                        ft.Text(f"{t_invest:,.2f} TK", size=20, weight="bold")
                    ]),
                    bgcolor="#1e2b5e", padding=20, border_radius=10, expand=True,
                ),
            ], spacing=20),
            ...
            ft.Container(
                content=ft.ListView(controls=[stock_table], expand=True),
                expand=True
            )
